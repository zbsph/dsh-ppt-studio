...
/**
 * 模板画廊（用户视觉选择，v0.14）：生成 self-contained gallery.html（网格卡片：
 * preview.png 全览 + COM 真渲染关键页 + 色板 + 使用指引），写入预览根
 * ~/.dsh/ppt-studio/preview/tpl-gallery/（复用 resolveToken 静态服务；资源经
 * /ppt-template-assets/<id>/<file> 白名单路由）。返回 { url, file }。
 */
export async function buildTemplateGallery() {
  const { writeFile, mkdir, rm } = await import('node:fs/promises')
  const { homedir } = await import('node:os')
  const galleryDir = join(homedir(), '.dsh', 'ppt-studio', 'preview', 'tpl-gallery')
  await rm(galleryDir, { recursive: true, force: true })
  await mkdir(galleryDir, { recursive: true })
  const list = await listTemplates()
  const cards = await Promise.all(list.map(async (t) => {
    const tplDir = join(TEMPLATES_DIR, t.id)
    const hasPreviews = existsSync(join(tplDir, 'previews')) && (await readdir(join(tplDir, 'previews'))).some((f) => /^0[15]\.png$/.test(f))
    const thumbs = []
    if (t.preview) thumbs.push({ src: `/ppt-template-assets/${encodeURIComponent(t.id)}/preview.png`, label: '总览' })
    if (hasPreviews) {
      thumbs.push({ src: `/ppt-template-assets/${encodeURIComponent(t.id)}/previews/01.png`, label: '封面' })
      thumbs.push({ src: `/ppt-template-assets/${encodeURIComponent(t.id)}/previews/05.png`, label: '内容页' })
    }
    const swatches = (t.colors ?? []).filter((c) => /^#/.test(c)).slice(0, 6)
    return `<div class="card">
  <div class="head"><span class="name">${escHtml(t.name)}</span><span class="id">${escHtml(t.id)}</span></div>
  <div class="meta">${escHtml(t.style || '')}${t.scene ? ' · ' + escHtml(t.scene) : ''}</div>
  <div class="thumbs">${thumbs.map((th) => `<div class="thumb"><img loading="lazy" src="${th.src}" alt="${th.label}"><span>${th.label}</span></div>`).join('')}</div>
  ${swatches.length ? `<div class="colors">${swatches.map((c) => `<span class="sw" style="background:${escHtml(c)}" title="${escHtml(c)}"></span>`).join('')}${swatches.map((c) => `<span class="hex">${escHtml(c)}</span>`).join('')}</div>` : ''}
  <div class="use">👉 在对话里说：<b>用「${escHtml(t.name)}」模板</b>${t.scene ? `（${escHtml(t.scene)}）` : ''}</div>
</div>`
  }))
  const html = `<!doctype html><html lang="zh"><head><meta charset="utf-8"><title>PPT 工作室 · 模板库</title>
<style>
body{margin:0;padding:24px;background:#0f172a;color:#e2e8f0;font-family:"Segoe UI","Microsoft YaHei",sans-serif}
h1{font-size:22px;margin:0 0 4px} .sub{color:#94a3b8;margin:0 0 20px;font-size:13px}
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:20px}
.card{background:#1e293b;border-radius:12px;padding:16px;border:1px solid #334155}
.head{display:flex;justify-content:space-between;align-items:baseline;gap:8px}.name{font-size:17px;font-weight:600}
.id{font-size:11px;color:#64748b}.meta{color:#94a3b8;font-size:12px;margin:4px 0 10px}
.thumbs{display:flex;gap:8px;overflow-x:auto}.thumb{min-width:120px;text-align:center}
.thumb img{width:100%;border-radius:6px;display:block;background:#0f172a;border:1px solid #334155}
.thumb span{font-size:11px;color:#94a3b8;display:block;margin-top:2px}
.colors{display:flex;gap:4px;align-items:center;margin:10px 0 4px;flex-wrap:wrap}
.sw{width:14px;height:14px;border-radius:3px;border:1px solid #475569}.hex{font-size:10px;color:#64748b;margin-right:6px}
.use{margin-top:8px;font-size:13px;color:#cbd5e1;background:#0f172a;padding:8px 10px;border-radius:6px}
</style></head><body>
<h1>PPT 工作室 · 模板库${list.length ? `（${list.length} 套）` : ''}</h1>
<p class="sub">点击卡片看图（整页真渲染）；选择后在对话里说出模板名即可生成工作区。用户模板 &gt; 内置模板 &gt; 从零定调。</p>
<div class="grid">${cards.length ? cards.join('') : '<p>（模板库为空）</p>'}</div>
</body></html>`
  await writeFile(join(galleryDir, 'gallery.html'), html)
  await writeFile(join(galleryDir, '.meta.json'), JSON.stringify({ sourceDir: 'template-gallery', token: 'tpl-gallery' }))
  return { url: '/ppt-preview/tpl-gallery/gallery.html', file: join(galleryDir, 'gallery.html'), templates: list.length }
}
...
/**
 * 模板资源白名单校验（画廊图引用 /ppt-template-assets/<id>/<file>）：
 * id 必须是现有模板；file 仅允许 preview.png / previews/0X.png（路径穿越防御：白名单正则）。
 * 返回绝对路径或 null。
 */
export async function validateTemplateAsset(id, file) {
  const safe = String(id ?? '')
  if (!safe || !/^[\w\u4e00-\u9fff-]+$/.test(safe)) return null
  if (typeof file !== 'string' || !/^(preview\.png|previews\/\d{2}\.png)$/.test(file)) return null
  const tplDir = join(TEMPLATES_DIR, safe)
  if (!existsSync(tplDir)) return null
  const p = join(tplDir, file)
