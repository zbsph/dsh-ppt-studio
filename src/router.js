...
export function workflowSection(taskType, cfg) {
  ...
  return {
    ...
    text: [
      ...
      '2. 每页制作后依次运行：ppt_render → ppt_verify（数字审阅）→ ppt_shot（有读图能力时视觉审阅）；overlap/out-of-page/text-overflow 错误清零后才算通过。**重叠语义（设计意图声明制）**：设计页面时把**有意**重叠的元素对记入页面 `expectedOverlaps`（图片上标注、色块衬底、箭头跨越等）；审阅时警告级重叠逐对与声明对照——命中 = 确认（✓ 预期重叠），未命中 = 设计预期外错误（修正布局，或确认是有意的 → 补声明再验）；**内容互压（文字/表格/图表相互遮挡）不支持声明豁免，永远错误**；纯装饰元素可加 `role: decoration` 豁免**重叠**（装饰性=设计意图声明）。**出界分级（与声明制同构）**：超页面边界=永远错误不可声明；超安全区（模板页眉页脚带）=声明制——有意落在带上的 logo/角标/水印逐元素写入页面 `expectedOutOfSafeArea`（不批量、必须手工，id 必须存在）；未命中=错误。declaration 之后若要免声明批量使用：仅重叠可用 `ppt_verify autoDeclare=true`（见入门段），出界声明永远手工。',
      ...
      '7. 交付前：S4 全通过 + S6 导出 .pptx + 交付说明（素材来源、未视觉审阅项、修改指引）。',
      ...
      '11. 质量档 audit（/ppt quality audit，从严门禁）：禁 autoDeclare（工具已拦）；每页必须 ppt_shot + 读图视觉审阅（无读图能力则降级结构 Lint 并在交付说明标注"未经视觉审阅"）；导出报告必须包含并核对 audit 回读断言（页数/尺寸/最小字号 ≥ minFontSize）；交付说明含素材来源与数据脱敏/假设标注。',
    ].join('\n'),
  }
}
