...
export function workflowSection(taskType, cfg) {
  if (cfg.quick) {
    return {
      ...
      text: [
        ...
        '5. 审阅：ppt_render + ppt_verify 必跑——内容互压/出界/文本溢出/预期外重叠（设计意图声明制）必须清零；跳过截图视觉审阅与美学迭代。',
  ...
  return {
    ...
    text: [
      ...
      '  - 批量声明：色块衬底/图片标注/箭头跨越等"警告级"未声明重叠，用 ppt_verify autoDeclare=true 一键写入 expectedOverlaps；内容互压（content-collision）不支持自动声明，必须手工改布局。',
      '',
      '【铁律】',
      '1. 产物 = 中间层项目（deck.yaml + pages/*.yaml + media/）。页面用元素描述（bounds 为 [x,y,w,h] px，1px=1pt，原点左上）；不要直接写 pptx。',
      '2. 每页制作后依次运行：ppt_render → ppt_verify（数字审阅）→ ppt_shot（有读图能力时视觉审阅）；overlap/out-of-page/text-overflow 错误清零后才算通过。**重叠语义（设计意图声明制）**：设计页面时把**有意**重叠的元素对记入页面 `expectedOverlaps`（图片上标注、色块衬底、箭头跨越等）；审阅时警告级重叠逐对与声明对照——命中 = 确认（✓ 预期重叠），未命中 = 设计预期外错误（修正布局，或确认是有意的 → 补声明再验）；**内容互压（文字/表格/图表相互遮挡）不支持声明豁免，永远错误**；纯装饰元素可加 `role: decoration` 完全豁免；草稿/旧项目可用页面级 `overlapMode: lenient` 缓冲（未声明仅提示）。',
      ...
      '10. 引擎：auto 默认 = pptd（自研主引擎，图表矢量拼绘）；python-pptx 仅用户在 /ppt engine 显式指定或 pptd 硬失败时使用（其图表降级为表格）——除非用户要求，不要自行切换引擎。',
    ].join('\n'),
  }
}
