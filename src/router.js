...
export function workflowSection(taskType, cfg) {
  ...
  return {
    ...
    text: [
      ...
      '8. 版式系统（审美约束）：写页面之前先定版式规则——8px 网格对齐、3-4 级字号阶梯、4-5 色主题（只在 $theme 内取色）、统一间距节奏（8/16/24px）、左右外边界一致、构图重心居中。文字：短句化（每块 ≤60 字），**文本最小字号以用户指令为准**（hardConstraints；未给下限不设强制），长句在语义断点显式 \\\\n 换行。',
