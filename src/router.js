...
export function workflowSection(taskType, cfg) {
  if (cfg.quick) {
    return {
      ...
      text: [
        ...
        '7. 引擎：auto 默认 = pptd（自研主引擎，图表矢量拼绘）；python-pptx 仅显式指定或 pptd 失败时使用（其图表为原生可编辑图表 + 内嵌数据工作簿）。不要自行切换引擎。',
  ...
  return {
    ...
    text: [
      ...
      '10. 引擎：**PPT 制作唯一通道 = pptd**（`deck.yaml` + `pages/*.yaml`；图表矢量拼绘）。pptd 硬失败时自动回退 python-pptx（报告醒目标注降级；其图表为**原生可编辑图表 + 内嵌数据工作簿**）；python-pptx 仅用户显式指定时使用。**不要**用 python-pptx 或官方 `office-pptx` 技能从零拼版来绕过本工作流的门禁——它们只适合**读取**既有 pptx、或作为上面的兜底（能不能用由 `ppt_state` 的 `capabilities` 如实报告）。除非用户要求，不要自行切换引擎。',
