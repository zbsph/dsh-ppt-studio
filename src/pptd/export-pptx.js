...
export async function exportPptx(ctx, { out = 'out.pptx', engine = 'pptd' } = {}) {
  ...
  // 媒体部件命名（2026-09-26 修复"同名静默错图"）——**整册共享**（不能按页各算一套，否则同一 srcPath
  // 在不同页会拿到不同部件名，parity 会假红）。要防的事故：旧实现 addMedia 按**完整路径**去重（rId 不同），
  // 但包内部件名与 rels Target 都取 `basename` ⇒ `media/a/logo.png` 与 `media/b/logo.png` 共用一个
  // `ppt/media/logo.png`，后写覆盖先写 —— **成品里一张图被静默换成另一张**，而 parity（只数 `<p:pic>`）与
  // warnings 全绿（2026-09-26 实测复现）。
  // 规则：ASCII 安全且未被占用 ⇒ **原名不动**（既有产物零变化）；非 ASCII/空格或重名 ⇒ `stem-<hash8>.<ext>`
  // （hash 取相对路径，跨机器/跨运行确定；`ppt/media/` 是包内部件名，用户看不到）。
  const usedParts = new Map() // srcPath → 部件名
  const takenParts = new Set()
  const renamedMedia = new Map() // srcPath → 改名后的部件名（供报告"不静默"）
  const mediaPartName = (srcPath) => {
    if (usedParts.has(srcPath)) return usedParts.get(srcPath)
    ...
    let part = base
    if (!/^[A-Za-z0-9._-]+$/.test(base) || takenParts.has(base)) {
      const slug = stem.replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'image'
      const h = createHash('sha1').update(key).digest('hex').slice(0, 8)
      part = `${slug}-${h}${ext}`
      let n = 1
      while (takenParts.has(part)) part = `${slug}-${h}-${n++}${ext}`
      renamedMedia.set(key, part)
    }
    takenParts.add(part)
    usedParts.set(srcPath, part)
    return part
  }
  ...
  for (const page of ctx.pages) {
    ...
    const addMedia = (srcPath) => {
      ...
      media.push({ srcPath, rId, part: mediaPartName(srcPath) })
      return rId
    }
  ...
  // 媒体 parity（2026-09-26 新增）：**不同 srcPath 数 == 包内媒体部件数**。
  // 为什么必须单独加一条：图数量 parity（imagesExp/imagesOut）只数 `<p:pic>`，两张不同的图塌成同一个部件时
  // 它依然是 2/2、ok=true（实测）。而"两张不同的图被写成同一个部件"= 成品里静默用错图，正是本插件最不能出的错。
...
function tableFrame(el) {
  ...
  const trs = rows.map((r, ri) => {
    const tds = Array.from({ length: ncols }, (_, ci) => {
      ...
      // 字号来自 normalizePage 的 fontPt（= max(11, theme.minFontSize)），不再硬编码 1100：
      // 硬编码会让 audit 档"最小字号 ≥ minFontSize"的断言对含表工程恒假，且用户在 DSL 里无法修正。
      const cellSz = Math.round((Number(el.fontPt) || 11) * 100)
      const rPr = '<a:rPr lang="zh-CN" sz="' + cellSz + '"' + (isH ? ' b="1"' : '') + '><a:solidFill><a:srgbClr val="1F2937"/></a:solidFill></a:rPr>'
