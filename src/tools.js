...
export function resolveEngine(engine) {
  ...
  if (engine === undefined || engine === null || engine === '' || engine === 'auto') {
    return { engine: 'pptd', allowFallback: true }
  }
  // 未知取值**不得静默当成 auto**（明文红线：不静默切换引擎）。
  // 历史形状：`/ppt engine pptxgenjs` 被命令面接受并回执"已记录（待接入）"，而这里把它静默映射成 pptd——
  // 用户以为在用 pptxgenjs，实际跑的是 pptd，且报告里一个字都不提。现在带上 `unknown`，由调用方明说。
  return { engine: 'pptd', allowFallback: true, unknown: String(engine) }
}
 ...
 */
/** 质量档 + **诊断**（2026-09-18 修）：读取失败**必须可见**，绝不静默吞掉。
 *  原来的实现是 `catch { return { quality: 'standard' } }`——审计档一旦因状态文件损坏而回落，
 *  用户看到的是一个"正常"的 standard 轮次，而 audit 档的禁 autoDeclare / 强制视觉审阅 / 回读断言全部消失。
 *  返回 `degraded` + `reason`，由调用方写进工具输出（诚实边界优于静默降级）。 */
export async function qualityOfDiag(agent, dir) {
  const notes = []
  let session = null
  let proj = null
  try {
    ...
    if (sid) {
      const s = await loadSessionDiag(sid)
      session = s.state
      if (s.error) notes.push(s.error)
    }
  } catch (e) {
    notes.push(`会话状态读取异常：${String(e?.message ?? e)}`)
  }
  try {
    const p = await loadProjectDiag(dir)
    proj = p.state
    if (p.error) notes.push(p.error)
  } catch (e) {
    notes.push(`项目状态读取异常：${String(e?.message ?? e)}`)
  }
  const q = proj?.quality === 'audit' || session?.quality === 'audit' ? 'audit' : (proj?.quality ?? session?.quality ?? 'standard')
  // engine 一并带出：`/ppt engine X` 写的是会话/项目档位，导出侧缺省时必须采纳它（见 ppt_export 注释）。
  const engine = proj?.engine ?? session?.engine ?? 'auto'
  return { quality: q, engine, degraded: notes.length > 0, reason: notes.join('；') }
}
 ...
 */
async function fallbackNote(ctx, r) {
  const src = String(r?.interpreterSource ?? '')
  const where = src.startsWith('bundled')
    ? `捆绑 Python（${r?.interpreter}；自带 python-pptx/lxml/Pillow/XlsxWriter）`
    : `PATH 解释器（${r?.interpreter ?? '?'}）`
  const notes = [`\n解释器：${where}`]
  try {
    const skills = await probeOfficeSkills(ctx)
    if (skills.available) {
      notes.push('本部署提供官方 `office-pptx` 技能（同一套捆绑 Python + LibreOffice kit 渲染 + `check_office.py` 结构自检）：'
        + '需要更细的读取/修改或渲染核对时**加载它**。注意它不改变引擎优先级——pptd 才是主通道，本产物是降级结果。')
    }
  } catch { /* 探测失败不影响导出 */ }
  return notes.join('\n')
}
...
export function registerTools(ctx) {
  ...
  reg({
    name: 'ppt_new',
    description: '一键生成可跑通全链路的样例 deck 工程（deck.yaml + 4 个页面，含主题 token / 色块衬底 / expectedOverlaps / safeArea 示例）。已存在 deck.yaml 时拒绝覆盖。可用 template=<id> 从内置模板库复制工作区（theme + 母版页）',
    parameters: {
  ...
  reg({
    name: 'ppt_crosscheck',
    description: '内容审阅材料包（M3 重写 2026-09-18）：把全页正文（阅读顺序，含表格/图表数据/讲稿/作者自述出处）+ 工作区实扫的外部素材清单 + 审阅协议（判定四分类：一致/冲突/无来源支撑/无法核实）端上桌，供独立子代理或模型做"前后一致 / 数据不冲突 / 无编造曲解"审阅。**工具本身不做任何判定、不进门禁**（旧版跨页数字分组与 grounded/unmapped 已废除：数字必须放回整句语境才有含义，source 是作者自述而非证据）',
    parameters: { dir: dirSchema },
    output: markdownResult(),
    async execute({ dir }) {
      try {
  ...
  reg({
    ...
    async execute(args, exec) {
      ...
      const pagesSel = typeof args.pages === 'string' ? parsePagesArg(args.pages) : null
      // 质量档 + 诊断（2026-09-18）：状态文件损坏导致的回落必须**可见**，不能悄悄按 standard 跑
      const qd = await qualityOfDiag(exec?.agent, dir)
      const degradeNote = qd.degraded
        ? `\n\n---\n⚠ **质量档降级**：${qd.reason}\n本轮按 ${qd.quality} 处理——audit 档的额外门禁（禁 autoDeclare / 强制视觉审阅 / 导出回读断言）**未生效**。修好状态文件，或重跑 /ppt quality <档>。`
        : ''
      try {
        if (autoDeclare) {
          if (blockedByAudit(qd.quality)) {
  ...
  reg({
    ...
    async execute({ dir, engine, out }, exec) {
      try {
        ...
        const qdE = await qualityOfDiag(exec?.agent, dir)
        const quality = qdE.quality
        // 引擎：工具参数优先；**缺省时采纳会话/项目档位**（`/ppt engine X` 设的就是它）。
        // 历史 bug：旧实现只看工具参数 ⇒ `/ppt engine python-pptx` 设完再用 ppt_export（不带参数）
        // 仍然跑 pptd —— 一个"设了没用"的档位，且报告里的 `引擎` 行还显示用户选的那个。
        const eff = resolveEngine(engine ?? (qdE.engine && qdE.engine !== 'auto' ? qdE.engine : undefined))
        const engineNote = eff.unknown
          ? `\n⚠ 引擎取值 "${eff.unknown}" 未被支持——已按 auto（pptd + 硬失败兜底）处理，**没有静默按你的选择跑**。可用：auto | pptd | python-pptx。`
          : ''
        const audit = blockedByAudit(quality)
        const withAudit = async (file, extra = '') => {
          let note = extra
          // 质量档回落必须写进导出报告（否则用户以为自己在 audit 档，而额外门禁全都没跑）
          if (qdE.degraded) {
            note += `\n⚠ **质量档降级**：${qdE.reason}\n本轮按 ${qdE.quality} 处理——audit 档的额外门禁（禁 autoDeclare / 强制视觉审阅 / 导出回读断言）**未生效**。`
          }
  ...
  reg({
    ...
    async execute({ dir }) {
      ...
      const pr = await loadProjectDiag(dir)
      const proj = pr.state
      const lines = [
      ...
      // 状态文件坏了必须说出来（否则"质量模式: standard"看起来是用户设的，其实是回落）
      if (pr.error) lines.push(`⚠ ${pr.error}——以上为默认值，不是你的设置`)
      return lines.join('\n')
    },
  })
