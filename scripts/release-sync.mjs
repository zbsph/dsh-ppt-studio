...
if (bundleMode) {
  ...
  // 非 bundle 模式（**历史遗留**：0.1.6 时代的"预设行 + 预设内 junction"）：挂载源 = 那条 junction → 部署副本。
  // 判据同样是"逐文件 sha256"，只是要比的是穿过 junction 读到的字节。
  // 【2026-09-18 修】此前这一支不设 mountMatch（恒为 null），而 ⑥ 的 LOCAL 判据要求 `mountMatch === true`
  // ⇒ 在本模式下 `--local` **恒定报 ❌**。写 `--local` 时只考虑了 bundle 模式，这是本模式的实际缺陷。
  // 【2026-09-24 注】DSH 0.1.7 起 `--isolate` 被安装器**明确拒绝**（目录发现已删除，写出来也不会被读），
  // 所以这一支只服务"升级前就已装成孤岛的老机器"，不再给出"重跑 --isolate"这种现在会失败的建议。
  ...
  const candidates = [
    { dir: join(dshHome, '.agent-presets', 'ppt', 'plugin'), label: '预设内 junction（0.1.6 时代的 --isolate）' },
  ...
  if (hit) {
    ...
    console.log(`⑤b 非 bundle 模式（历史遗留装法）：挂载源 = 刚部署的副本（${hit.label}）\n    **穿过 junction** 逐文件 sha256 自证：${mountMatch ? '与部署副本同源 ✓' : '与部署副本不一致 ⚠'}（lib/index.js · package.json · cordis.patch.yml）`)
    if (!mountMatch) console.log(`    ⚠ 不一致 → 建议**改回默认装法**（DSH 0.1.7 上 --isolate 已不可用）：dsh plugin --profile ${profileName} add "${installSpec}"，然后 node "${join(deployRoot, 'package', 'scripts', 'install.mjs')}"`)
  } else {
    mountMatch = false
    console.log(`⑤b 非 bundle 模式（历史遗留装法）：**未找到挂载链接**（预设内 ${join(dshHome, '.agent-presets', 'ppt', 'plugin')} / profile ${join(dshHome, 'profiles', profileName, 'node_modules', ...pkg.name.split('/'))} 都不存在）——请改用默认装法：dsh plugin --profile ${profileName} add "${installSpec}"，然后 node "${join(deployRoot, 'package', 'scripts', 'install.mjs')}"`)
  }
}
