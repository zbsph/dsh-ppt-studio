...
```powershell
node scripts/smoke.mjs          # 220 断言（含全链路）
...
npm test                        # build + smoke（220 断言）+ 预设漂移自检（DSH 升级后必跑）
...
- **既有的自动化验证**：smoke（220 断言，全链路）→ preflight（发布预检）→ regression-real（真实资产）→ preset 自检（DSH 升级后）→ 手册事实审计（`audit-manual-facts.mjs`）→ 真实任务闭环（参考 docs/06 的测试矩阵与历轮反馈）。
