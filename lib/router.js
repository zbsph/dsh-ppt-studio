...
export function workflowSection(taskType, cfg) {
  if (cfg.quick) {
    return {
      ...
      text: [
        ...
        '2. 版式：用中性三件套骨架（封面/内容/结尾），**不套内置模板**、不做视觉定调；配色与字号按本册主题（按题材定）走，下限以用户指令为准（未给下限不做强制）。',
        ...
        '7. 引擎：auto 默认 = pptd（自研主引擎；**图表默认原生可编辑**：图表部件 + 内嵌数据工作簿，`chart.render: vector` 可回到矢量拼绘）；python-pptx 仅显式指定或 pptd 失败时使用（其图表同为原生可编辑图表 + 内嵌数据工作簿）。不要自行切换引擎。',
  ...
  return {
    ...
    text: [
      ...
      '10. 引擎：**PPT 制作唯一通道 = pptd**（`deck.yaml` + `pages/*.yaml`；**图表默认原生可编辑**：图表部件 + 内嵌数据工作簿，`chart.render: vector` 可回到矢量拼绘）。pptd 硬失败时自动回退 python-pptx（报告醒目标注降级；其图表同为**原生可编辑图表 + 内嵌数据工作簿**）；python-pptx 仅用户显式指定时使用。**不要**用 python-pptx 或官方 `office-pptx` 技能从零拼版来绕过本工作流的门禁——它们只适合**读取**既有 pptx、或作为上面的兜底（能不能用由 `ppt_state` 的 `capabilities` 如实报告）。除非用户要求，不要自行切换引擎。',
      ...
      '13. **改既有 PPT 的两条纪律**：① **就地手术优先**——任务若是"改某几页 / 改一处文字 / 换一张图"，优先 `ppt_splice`（把工作区某一页替换进原稿，**其余页逐字节不变**；用户要"单页独立版"用 `ppt_slice`；要"看起来就是模板原样"用 `ppt_patch`），不要图省事整册重渲。`ppt_import` 会给出**页码/元素级**的"无法保真特性"清单（工程内 `loss-audit.json` + 一行结论），并按 ✓/⚠/ℹ 三档区分：✓ 干净、⚠ 有会丢的可见特性（动画/阴影/超链接/切换/SmartArt/EMF 等）、ℹ 仅主题样式表里的渐变模板未被引用（不影响观感）；**⚠ 命中时"只改某几页"必须走 `ppt_splice`，并在交付说明里写明"整册重渲会丢 <类别 N 处>"**。② **Excel 素材的数字要么重算、要么标注**：从 xlsx 取数时公式的缓存值可能陈旧或缺失；探测到捆绑 LibreOffice kit 就用它的 `recalculate` 重算，做不到时必须在交付说明里写明"公式值未重算（可能陈旧）"，别把旧缓存值当已核对的数据。',
    ].join('\n'),
  }
}
