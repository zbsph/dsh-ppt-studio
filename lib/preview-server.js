 ...
 * - 预览根 = `<dshHome>/ppt-studio/preview/<token>/`：pages/（render 的 html）+ media/（deck 媒体）。
 *   render 输出的 html 引用 `../media/`（相对 deck 根）→ 预览根内 pages/../media/ 恰好解析。
 *   **路径每次调用求值**（认 `DSH_HOME`；见 src/home.js 的口径统一说明）。
...
/** 预览根（调用时求值）。 */
export function previewRoot() { return join(pptStudioDir(), 'preview') }
...
export async function buildPreview(dir) {
  ...
  const root = join(previewRoot(), token)
