 ...
 *  - ERROR table-overflow 表格单元格内容按列宽换行后超出表格高度（2026-09-18 新增：表格此前是内容门禁的盲区）
...
export function analyzePage(page, size) {
  ...
  for (const el of els) {
    ...
    // 溢出判定（wrap=true 只判垂直；wrap=false 判水平）
    // 【2026-09-18】表格并入此处：表格是 role:content 的内容元素，其单元格文字溢出此前**完全不受门禁**——
    //   快照里只有 cols/rows 的**长度**，而本判定只认 kind==='text' ⇒ "该严的地方没严"。
    //   现在 layout.js 按列宽换行给出表格的 overflowY（与正文同一保守度量），这里用**独立错误码**
    //   `table-overflow`（与 ⑥ 同一条理由：处置不同就该分码——表格的杠杆是行高/列宽/文案，不是文本框）。
    //   影响面已实测：仓库全部夹具 2 张表、0 张被判溢出（见 docs/03 本轮记录），故对既有工程零影响。
    if (el.kind === 'text' || el.kind === 'table') {
      const m = el.metrics ?? {}
      const isTable = el.kind === 'table'
      const wrap = isTable ? true : el.style?.wrap !== false
      const ox = m.overflowX ?? 0
      const oy = m.overflowY ?? 0
      if (wrap && oy > TOL) {
        if (isTable) {
          // 独立分支而非三元：错误码提取器（smoke §37.2/§37.7）是**文本启发式**，只认 `code: '字面量'`——
          // 写成 `code: a ? 'x' : 'y'` 会让两个码同时从守卫视野里消失（本轮实际踩到）。
          findings.push({
            severity: 'error', code: 'table-overflow', id: el.id,
            message: `表格 "${el.id}" 内容溢出 ${oy}px（按列宽估算内容高 ${Math.round(m.textH ?? 0)}px / 表格高 ${b.h}px，共 ${m.lines ?? 0} 行）——扩大表格高度或精简单元格文案`,
            area: b,
          })
        } else {
          findings.push({
...
export function verifyDeck(layout) {
  ...
  let total = 0
  // 结构化 findings（2026-09-26 新增，**纯增量**）：消费方（tools.js 的门禁计数、templates.js 的收纳报告）
  // 此前靠 `text.split('\n').filter(l => l.includes('[✗]'))` 与正则 `\[✗\] (\w+)` 反解文本 —— 后者会把
  // `out-of-page` 截成 `out`（`\w` 不含连字符），报告里的错误码直方图全是错的。这里把本就算出来的
  // errors/warns/suggestions 直接带出去，文本格式保持原样（`text` 对用户与旧断言零变化）。
  const allErrors = []
  const allWarns = []
  ...
  for (const page of layout.pages ?? []) {
    ...
    const at = { page: page.index + 1, pageName: page.name }
    allErrors.push(...errors.map((f) => ({ ...f, ...at })))
    allWarns.push(...warns.map((f) => ({ ...f, ...at })))
    allSuggestions.push(...suggestions.map((s) => ({ ...s, ...at })))
  ...
  return { text: out.join('\n'), total, size, errors: allErrors, warns: allWarns, suggestions: allSuggestions }
}
